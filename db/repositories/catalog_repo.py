# app/db/repositories/catalog_repo.py

from typing import List, Dict, Any, Optional, Sequence
from sqlalchemy import select, func, desc
from sqlalchemy.orm import Session, aliased, joinedload

from db.models.masters import MastersMake, MastersModel, MastersVariant, VariantSpecs, VariantPriceHistory
...
def fetch_active_variants_with_specs(
    ...
    *,
    ...
    stmt = (
        select(
            MastersMake.id.label("make_id"),
            MastersMake.name.label("make"),
            MastersMake.is_active_india.label("make_active"),
            MastersMake.brand_perception_score,
            MastersMake.brand_resale_score,
            MastersModel.id.label("model_id"),
            MastersModel.name.label("model"),
            MastersModel.body_type.label("body_type"),
            MastersModel.launch_year,
            MastersModel.last_active_year,
            MastersModel.ncap_rating,
            MastersVariant.id.label("variant_id"),
            MastersVariant.name.label("variant"),
            MastersVariant.launch_year,
            MastersVariant.discontinue_year,
        ...
        .join(MastersModel, MastersModel.make_id == MastersMake.id)
        .join(MastersVariant, MastersVariant.model_id == MastersModel.id)
        .join(VariantSpecs, VariantSpecs.variant_id == MastersVariant.id)
        .join(latest_price_sq, latest_price_sq.c.variant_id == MastersVariant.id)
        .where(MastersMake.is_active_india)
        .where((MastersModel.last_active_year.is_(None)) | (MastersModel.last_active_year >= current_year))
        .where((MastersVariant.discontinue_year.is_(None)) | (MastersVariant.discontinue_year >= current_year))
        .order_by(desc(latest_price_sq.c.ex_showroom_price_inr))
        .limit(limit)
    )

    if allowed_body_types:
        stmt = stmt.where(MastersModel.body_type.in_(allowed_body_types))
